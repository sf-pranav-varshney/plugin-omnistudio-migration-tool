...
export default class VlocityDatePicker extends LightningElement {
  ...
  get errorClass() {
    let errorclass = `${this.theme}-form-element ${this.theme}-size_1-of-1 ${this.theme
      }-dropdown-trigger ${this.theme}-dropdown-trigger_click ${this.isError ? this.theme + "-has-error" : ""
      }`;
    return errorclass;
  }
  ...
  setThePrintLoopAux(s, d, n, y, m) {
    ...
    for (let i = 0; i < 42; i++) {
      ...
      let dayObj = {
        ...
        disabled,
        ariaLabel: new Date(
          `${this.months[monthIdxAndYearVal.monthIdx]} ${val} ${monthIdxAndYearVal.year
          }`
        ).toDateString()
      };
  ...
  handleReadOnlyCalenderOpenInMobile() {
    ...
    const readOnlyDate = this.template.querySelector('.datepicker_input__container');
    if (readOnlyDate) {
