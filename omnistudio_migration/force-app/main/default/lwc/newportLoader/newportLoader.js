...
export function load(component, useRTL=false) {
    if (!isLoaded) {
        ...
        return omniRestApi.getNewportUrl().then(url => {
            ...
            let path = '/assets/styles/vlocity-newport-design-system-scoped.' + (url === 'isRTL' ? 'rtl.' : '') + 'min.css'
            return loadStyle(component, (url && url !== 'isRTL') ? url : omniRestApi.newport() + path);
        });
    }
